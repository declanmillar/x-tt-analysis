...
KinematicReconstructer::KinematicReconstructer(const double bMass, const double WMass):
    m_bMass(bMass),
    m_WMass(WMass)
{
    this->Reset();
...
bool KinematicReconstructer::Reconstruct(
    const std::pair< TLorentzVector, TLorentzVector >& p_l, const std::pair< TLorentzVector, TLorentzVector >& p_b,
    const TLorentzVector& p_miss, const double TopMass)
{
    // takes a pair of leptons, and pair of b-jets
    // (presumed to be already matched)
    // collects possible solutions
    ...
    std::vector<std::pair<TLorentzVector, TLorentzVector> > p_nu_sol;

    p_nu_sol = GetSolutions(p_l.first, p_l.second, p_b.first, p_b.second, p_miss);

    if (p_nu_sol.size() < 1) return false;

    if (m_debug) std::cout << "selecting best solution ...\n";

    int imin = 0;
    double mtt_min = DBL_MAX;
    for (int i = 0; i < p_nu_sol.size(); i++)
    {
        double mtt = (p_l.first + p_l.second + p_b.first + p_b.second + p_nu_sol.at(i).first + p_nu_sol.at(i).second).M();
        if (mtt < mtt_min)
        {
            mtt_min = mtt;
            imin = i;
    ...
    m_nu = p_nu_sol.at(imin).first;
    m_nubar = p_nu_sol.at(imin).second;
    m_b = p_b.first;
    m_bbar = p_b.second;
    ...
    if (m_debug)
    {
        if (m_top.M() - m_TopMass > 1.0 or m_tbar.M() - m_TopMass > 1.0)
        {
            std::cout << "Top masses " << m_top.M() << " " << m_tbar.M() << "\n";
            std::cout << "W boson masses " << (p_l.first + m_nu).M() << " " << (p_l.second + m_nubar).M() << "\n";
            std::cout << "Neutrinos masses" << m_nu.M() << " " << m_nubar.M() << "\n";
            std::cout << "Number of solutions = " << p_nu_sol.size() << "\n";
            std::cout << "imin = " << imin << "\n";
            std::cout << "\n";
        }
    }
...
bool KinematicReconstructer::ReconstructMulti(
    ...
    const double TopMass)
{
    ...
    std::vector<std::vector<std::pair<TLorentzVector, TLorentzVector> > > p_nu_sol;

    // solve quartic for each possible b-jet combination
    for (int i = 0; i < n_b; i++)
        for (int j = 0; j < n_b; j++)
            if (i != j) p_nu_sol.push_back(this->GetSolutions(p_l.first, p_l.second, p_b[i], p_b[j], p_miss));

    if (m_debug) std::cout << "selecting best solution ...\n";
    if (m_debug) std::cout << "number of b-combinations: " << p_nu_sol.size() << "\n";

    int nsols = 0;
    for (int i = 0; i < p_nu_sol.size(); i++)
    {
        if (m_debug) std::cout << "number of real solutions for b-combo " << i << ": " << p_nu_sol.at(i).size() << "\n";
        nsols += p_nu_sol.at(i).size();
    }
    if (m_debug) std::cout << "total number of solutions = " << nsols << "\n";
    if (nsols == 0) return false;

    int I = 0, J = 0, K = 0, L = 0, l = 0;
    // int k = isol, l = combination of i and j
    double mtt_min = DBL_MAX;
    for (int i = 0; i < n_b; i++)
    {
        for (int j = 0; j < n_b; j++)
        {
            if (i != j)
            {
                for (int k = 0; k < p_nu_sol.at(l).size(); k++)
                {
                    double mtt = (p_l.first + p_l.second + p_b[i] + p_b[j] + p_nu_sol.at(l).at(k).first + p_nu_sol.at(l).at(k).second).M();
                    if (mtt < mtt_min)
                    {
                        mtt_min = mtt;
                        I = i; J = j; K = k; L = l;
    ...
    m_nu = p_nu_sol.at(L).at(K).first;
    m_nubar = p_nu_sol.at(L).at(K).second;
    m_b = p_b.at(I);
    m_bbar = p_b.at(J);
    m_top = m_b + p_l.first + m_nu;
    m_tbar = m_bbar + p_l.second + m_nubar;
    ...
    if (m_debug)
    {
        if (m_top.M() - m_TopMass > 1.0 or m_tbar.M() - m_TopMass > 1.0)
        {
            std::cout << "tops      " << m_top.M() << " " << m_tbar.M() << "\n";
            std::cout << "W bosons  " << (p_l.first + m_nu).M() << " " << (p_l.second + m_nubar).M() << "\n";
            std::cout << "neutrinos " << m_nu.M() << " " << m_nubar.M() << "\n";
            std::cout << "number b-combinations: " << p_nu_sol.size() << "\n";
            for (int i = 0; i < p_nu_sol.size(); i++)
            {
                std::cout << "number of real solutions for b-combo " << i << ": " << p_nu_sol.at(i).size() << "\n";
            }
    ...
    const TLorentzVector& p_miss)
{
    ...
    // double a[4] = { static_cast< double >(h1 / h0), static_cast< double >(h2 / h0), static_cast< double >(h3 / h0), static_cast< double >(h4 / h0) };
    double a[4] = { h1 / h0, h2 / h0, h3 / h0, h4 / h0 };
