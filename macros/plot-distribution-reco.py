...
class HistPainter():
    ...
    def AddInfoBox(self, model, xmass, xratio, energy, luminosity, efficiency, x, y):
        ...
        t = ROOT.TLatex(x, y - 0.21, "#bf{#it{#Gamma_{Z'} / m_{Z'}}  =  %.1f %%}" % xratio)
        t.SetTextAlign(11)
        t.SetTextSize(0.04)
        t.SetNDC(True)
        t.Draw("same")
        ...
        t = ROOT.TLatex(x, y - 0.07, "#bf{#it{#sqrt{s}}  =  %i TeV}" % (energy))
        t.SetNDC(True)
        t.SetTextSize(0.04)
        t.Draw("same")
        self.members.append(t)

        t = ROOT.TLatex(x - 0.007, y - 0.14, "#bf{#it{m_{Z'}}  =   %i TeV}" % 4)
        t.SetNDC(True)
        t.SetTextSize(0.04)
        t.Draw("same")
        self.members.append(t)
...
art.AddHistogram(hs[1], fs[0], " #bf{reconstruction}", black, 2)

art.AddInfoBox(models[0].rsplit('-',1)[0], 4, 2.5, 13, 300, 0.6, 0.77, 0.87)
art.AddLegend(0.15, 0.62, 0.42, 0.92)

art.Save("~/Desktop/" + hs[0] + "-" + "truth-vs-reco.pdf")
# i += 1
# j += 1
